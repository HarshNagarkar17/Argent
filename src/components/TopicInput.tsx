...
const TopicInput: React.FC = () => {
  const { 
    topic, setTopic, 
    userOpinion, setUserOpinion, 
    canAdvance, setCurrentStage,
    selectedAgents, addMessage
  } = useDebate();

  const handleStartDebate = () => {
    // Add the user's topic and opinion as the first message
    ...
    <div className="h-screen overflow-y-auto bg-[#36393f]">
      ...
      <div className="container max-w-2xl py-6 px-4 animate-fade-in">
        <Card className="border-[#202225] bg-[#2f3136] shadow-lg">
          <CardHeader>
            <CardTitle className="text-center text-white">What would you like to debate?</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              <label htmlFor="topic" className="text-sm font-medium text-[#dcddde]">
                Debate Topic
              </label>
              <Input
                id="topic"
            ...
            <div className="space-y-2">
              <label htmlFor="opinion" className="text-sm font-medium text-[#dcddde]">
