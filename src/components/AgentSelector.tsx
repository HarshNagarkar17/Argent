...
const AgentSelector: React.FC = () => {
  const { selectedAgents, toggleAgentSelection, canAdvance, setCurrentStage } =
    useDebate();
      ...
      <div className="container max-w-4xl py-6 px-4 animate-fade-in">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold mb-3 text-white">
            Select Your Debate Agents
          </h1>
          <p className="text-[#b9bbbe]">
            Choose 2-5 agents with different perspectives for your debate
          </p>
        </div>
        ...
        <div className="flex flex-col items-center">
          <p className="text-[#b9bbbe] text-sm mb-3">
            {selectedAgents.length > 0 ? (
              <>
                Selected {selectedAgents.length} agent
                {selectedAgents.length !== 1 ? "s" : ""}
                {selectedAgents.length >= 2 && selectedAgents.length <= 5
                  ? " - Good to go!"
                  : ""}
